...
fp_LoadLibraryA pfnLoadLibraryA = nullptr;
fp_PeekMessageA pfnPeekMessageA = nullptr;


HMODULE WINAPI hook_LoadLibraryA(LPCSTR lpLibFileName)
{
	OutputDebugStringA("hook_LoadLibraryA \n");
...
{
	OutputDebugStringA("hook_PeekMessageA \n");
	return pfnPeekMessageA(lpMsg, hWnd, wMsgFilterMin, wMsgFilterMax, wRemoveMsg);
}
...
* @return TRUE on success, otherwise FALSE
*/
int hookEATwithName(const char *modName, const char *targetName, DWORD hookFunc, DWORD* OldFunc)
{
	HMODULE hMod = NULL;
	ULONG   size = 0;
...
* @return TRUE on success, otherwise FALSE
*/
int hookEATwithAddress(const char *modName, DWORD targetFunc, DWORD hookFunc, DWORD* OldFunc)
{
	HMODULE hMod = NULL;
	ULONG   size = 0;
	PIMAGE_EXPORT_DIRECTORY pExportDirectory = NULL;
...
int EATHookTest()
{
	...
	hookEATwithName("kernel32.dll", "LoadLibraryA", (DWORD)hook_LoadLibraryA, (DWORD*)&pfnLoadLibraryA);
	...
	hookEATwithAddress("user32.dll", (DWORD)PeekMessageA, (DWORD)hook_PeekMessageA, (DWORD*)&pfnPeekMessageA);
	fp_PeekMessageA fp2 = (fp_PeekMessageA)getApiAddress("user32.dll", "PeekMessageA");
	fp2(NULL, NULL, 0, 0, 0);

	//hookEATwithAddress("user32.dll", (DWORD)PeekMessageA, (DWORD)pfnPeekMessageA, (DWORD*)&pfnPeekMessageA);
