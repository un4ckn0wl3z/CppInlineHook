...
HMODULE WINAPI IAThook_LoadLibraryA(LPCSTR lpLibFileName)
{
	OutputDebugStringA("hook_LoadLibraryA By IAT \n");
	return orig_LoadLibraryA(lpLibFileName);
}

BOOL WINAPI IAThook_PeekMessageA(LPMSG lpMsg, HWND hWnd, UINT wMsgFilterMin, UINT wMsgFilterMax, UINT wRemoveMsg)
{
	OutputDebugStringA("hook_PeekMessageA By IAT \n");
...
int IATHookTest()
{
	...
	//// Hook LoadLibraryA
	printf("%s: direct  : 0x%x, orig: 0x%x, hookFunc: 0x%x \n", "LoadLibraryA", (DWORD)LoadLibraryA, (DWORD)orig_LoadLibraryA, (DWORD)IAThook_LoadLibraryA);
	hookIATwithName(NULL, "LoadLibraryA", (DWORD)IAThook_LoadLibraryA, (DWORD *)&orig_LoadLibraryA);
	...
	//// Hook PeekMessageA
	printf("%s: direct  : 0x%x, orig: 0x%x, hookFunc: 0x%x \n", "PeekMessageA", (DWORD)PeekMessageA, (DWORD)orig_PeekMessageA, (DWORD)IAThook_PeekMessageA);
	//hookIATwithName(NULL, "PeekMessageA", (DWORD) hook_PeekMessageA, (DWORD *) &orig_PeekMessageA);
	hookIATwithAddress(NULL, (DWORD)PeekMessageA, (DWORD)IAThook_PeekMessageA, (DWORD *)&orig_PeekMessageA);
